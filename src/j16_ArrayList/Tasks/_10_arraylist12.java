...
public class _10_arraylist12 {
    ...
    public static void main(String[] args) {
        List<Integer> sayi = new ArrayList<>(List.of(1, 5, 22, 10, 20, 14, 8, 25, 30, 28));
        int min = 20;
        int max = 30;
        int count=0;
        System.out.println(rangeBtw(sayi, min, max, count));
    }

    private static int rangeBtw(List<Integer> sayi, int min, int max, int count) {
        for (int i = 0; i < sayi.size(); i++) {
            if (sayi.get(i)>=20 && sayi.get(i)<=30) count++;
