...
public class C05_sorted {
    ...
    private static void tekKareBKPrint(List<Integer> sayi) {
        sayi.stream().filter(t->t%2==1).map(t->t*t).sorted(Comparator.reverseOrder()).forEach(C01_LambdaExpression::yazdır);
/*
sayi.
                stream().
                filter(t->t%2==1).
 ...
 */
    ...
    public static void ciftKareKbPrint(List<Integer> sayi) {
        sayi.stream().filter(t->t%2==0).map(t->t*t).sorted().forEach(C01_LambdaExpression::yazdır);
